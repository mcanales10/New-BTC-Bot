...
export class WebSocketOrderBook {
    ...
    private tokenPairPartner: Map<string, string> = new Map(); // Map tokenId to its pair (Up<->Down)
    ...
    /**
     * Set Up/Down token pair for combined logging
     */
    setTokenPair(upTokenId: string, downTokenId: string): void {
        this.tokenPairPartner.set(upTokenId, downTokenId);
        this.tokenPairPartner.set(downTokenId, upTokenId);
    }
    ...
    private handleBestBidAsk(message: any): void {
        ...
        // Log Up and Down ask prices on one line when we have a pair
        if (bestAsk !== null) {
            const partnerId = this.tokenPairPartner.get(assetId);
            if (partnerId) {
                const upId = this.tokenLabels.get(assetId) === "Up" ? assetId : partnerId;
                const downId = this.tokenLabels.get(assetId) === "Down" ? assetId : partnerId;
                const upAsk = this.tokenPrices.get(upId)?.bestAsk ?? null;
                const downAsk = this.tokenPrices.get(downId)?.bestAsk ?? null;
                const upStr = upAsk !== null ? upAsk.toFixed(4) : "--";
                const downStr = downAsk !== null ? downAsk.toFixed(4) : "--";
                logger.info(`📊 Up Ask: ${upStr} | Down Ask: ${downStr}`);
            } else {

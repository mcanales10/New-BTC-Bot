...
/** Suppress noisy CLOB client "Could not create api key" errors from display and logs */
function shouldSuppressClobApiKeyError(raw: string): boolean {
    return (
        raw.includes("[CLOB Client] request error") &&
        raw.includes("Could not create api key")
    );
}
...
export function setupConsoleFileLogging(options: SetupOptions): void {
    ...
    (process.stdout.write as any) = (
        ...
        if (shouldSuppressClobApiKeyError(raw)) {
